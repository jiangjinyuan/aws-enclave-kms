...
func pad(data []byte, blocklen int) ([]byte, error) {
	if blocklen < 1 {
		return nil, errors.New("invalid block len " + strconv.FormatInt(int64(blocklen), 10))
	}
...
func unpad(data []byte, blocklen int) ([]byte, error) {
	if blocklen < 1 {
		return nil, errors.New("invalid blocklen " + strconv.FormatInt(int64(blocklen), 10))
	}
	if len(data)%blocklen != 0 || len(data) == 0 {
		return nil, errors.New("invalid data len " + strconv.FormatInt(int64(len(data)), 10))
	}
